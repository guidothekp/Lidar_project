...
std::vector<Node*> inOrderList(KdTree* tree) {
...
void proximity(int currentIndex, const std::vector<std::vector<float>> & points, 
        std::vector<int> & cluster, 
        float width, bool* processed, KdTree* tree) {
    processed[currentIndex] = true;
    cluster.push_back(currentIndex);
    std::vector<int> nearby = tree->search(points[currentIndex], width);
    for (int index : nearby) {
        if (processed[index]) {
            continue;
        }
        proximity(index, points, cluster, width, processed, tree);
    }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    ...
    for (int i = 0; i < points.size(); i ++) {
        ...
        std::vector<int> cluster;
        proximity(i, points, cluster, distanceTol, processed, tree);
